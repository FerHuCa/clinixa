...
public static class ReviewsEndpoints
{
        ...
        Func<IReadOnlyList<ReviewDto>, int, int, bool, IResult> applyPagination)
    {
        ...
        professionalsApi.MapGet("/{id}/reviews", async (HttpRequest httpRequest, string id, HealthHubDbContext db) =>
        {
            ...
            var entities = await db.Reviews
                .AsNoTracking()
                .Where(review => review.ProfessionalId == id && review.Status == "published")
                .OrderByDescending(review => review.CreatedAt)
                .ToListAsync();

            // Anonimiza PatientName antes de devolver a llamadores no autenticados (CWE-359).
            // El nombre completo se conserva en la BD; solo la proyeccion publica se enmascara.
            var publicReviews = entities.Select(review => review.ToPublicDto()).ToList();

            return paginated
                ? Results.Ok(PagedResult<PublicReviewDto>.From(publicReviews, page, pageSize))
                : Results.Ok(publicReviews);
        });
