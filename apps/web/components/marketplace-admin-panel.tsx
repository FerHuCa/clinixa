  ...
  return (
    <Panel title="Marketplace Mercado Pago">
      <div className="divide-y divide-border">
          ...
          <div
            className={
              feedback.kind === "success"
                ? "m-4 rounded-md border border-emerald-200 bg-emerald-50 px-3 py-2 text-sm text-emerald-700"
                : "m-4 rounded-md border border-rose-200 bg-rose-50 px-3 py-2 text-sm text-rose-700"
            }
          >
            {feedback.text}
          </div>
        ...
        {loading ? (
          <div className="p-4 text-sm text-slate-500">Cargando cuentas pendientes...</div>
        ) : pending.length > 0 ? (
          pending.map((item) => (
          ...
          <div className="p-4 text-sm text-slate-500">No hay cuentas pendientes de verificación.</div>
        )}
      </div>
    </Panel>
  );
